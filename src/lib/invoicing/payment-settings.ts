...
const DEFAULT_PAYMENT_METHODS = ["us_bank_account", "card"] as const;
...
/** Invoice payment methods: ACH first by default; set to "dashboard" to use Stripe defaults. */
export function getInvoicePaymentSettings():
  ...
  const paymentMethodTypes = parsePaymentMethodTypes(raw || DEFAULT_PAYMENT_METHODS.join(","));
